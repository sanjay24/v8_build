...
compiler::TNode<JSObject> ObjectBuiltinsFromDSLAssembler::AllocateEmptyJSObject(compiler::TNode<Context> p_context) {
  ...
  if (block0.is_used()) {
    ...
    USE(tmp2);
    tmp2 = ca_.UncheckedCast<Object>(CodeStubAssembler(state_).LoadJSFunctionPrototypeOrInitialMap(compiler::TNode<JSFunction>{tmp1}));
    compiler::TNode<Map> tmp3;
    USE(tmp3);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp3 = BaseBuiltinsFromDSLAssembler(state_).Cast5ATMap(compiler::TNode<Context>{tmp0}, compiler::TNode<Object>{tmp2}, &label0);
  ...
  return compiler::TNode<JSObject>{tmp17};
}
...
compiler::TNode<JSObject> ObjectBuiltinsFromDSLAssembler::ObjectFromEntriesFastCase(compiler::TNode<Context> p_context, compiler::TNode<Object> p_iterable, compiler::CodeAssemblerLabel* label_IfSlow) {
  ...
  if (block5.is_used()) {
    ...
    USE(tmp12);
    tmp12 = ca_.UncheckedCast<FixedArrayBase>(CodeStubAssembler(state_).LoadElements(compiler::TNode<JSObject>{tmp11}));
  ...
  if (block13.is_used()) {
    ...
    USE(tmp70);
    tmp70 = ca_.UncheckedCast<BoolT>(BaseBuiltinsFromDSLAssembler(state_).Is12ATJSReceiver22UT12ATHeapObject5ATSmi(compiler::TNode<Context>{tmp58}, compiler::TNode<Object>{tmp68}));
  ...
  return compiler::TNode<JSObject>{tmp115};
}
